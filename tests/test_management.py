...
def test_execute_from_command_line(test_project, waf):
    ...
    jawaf.management.discover()
    import sys
    sys.argv = [
        '', 'start-app', 'oranges', '--directory=temp_test/test_project']
...
def test_execute_from_command_line_bad_command(test_project, waf):
    ...
    assert os.path.exists(
        os.path.abspath('temp_test/test_project/uhoh')) is False


def test_run_command(test_project, waf):
    """Test run command"""
    commands = jawaf.management.discover()
    jawaf.management.run_command(
        commands['start_app'],
        ['', '', 'apples', '--directory=temp_test/test_project'])
